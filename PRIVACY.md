...
**1. Fetching the sources you configure.** At the refresh interval you choose, SmartTicker requests the public web pages you have added. These are read-only requests (HTTP GET). No personal information, cookies, or credentials are sent — cookie storage and credentials are disabled in the application. Requests identify the application with the user agent `SmartTicker/0.1 (+local desktop public HTML reader)`.
